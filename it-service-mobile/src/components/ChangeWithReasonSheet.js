 ...
 * onConfirm({ value, reasonCode, note }) çağrılır.
 */
export default function ChangeWithReasonSheet({
  visible,
  title,
  valueLabel,
  valuePlaceholder,
      ...
      <SheetBackdrop>
        <View style={[styles.sheet, { backgroundColor: theme.bgSurface }]}>
          ...
          <View style={styles.actions}>
            ...
            <Pressable
              onPress={() => value != null && code && onConfirm({ value, reasonCode: code, note: note.trim() })}
              disabled={value == null || !code || busy}
              style={[styles.btn, { backgroundColor: theme.primary, opacity: value == null || !code || busy ? 0.5 : 1 }]}
            >
              {busy ? (
