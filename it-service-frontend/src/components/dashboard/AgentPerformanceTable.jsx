    ...
    <section className="rounded-3xl border p-6 shadow-sm" style={{ backgroundColor: 'var(--bg-surface)', borderColor: 'var(--border-color)' }}>
      <div className="mb-5 flex items-start justify-between gap-4">
        <div>
          ...
          <h2 className="mt-3 text-lg font-bold" style={{ color: 'var(--text-primary)' }}>Performans sıralaması</h2>
          <p className="mt-1 text-sm" style={{ color: 'var(--text-secondary)' }}>
            Aktif iş yükü, son 24 saat çözüm sayısı, ortalama çözüm süresi ve CSAT tek tabloda.
          </p>
        </div>
        ...
        <div className="grid gap-2 text-right text-xs sm:grid-cols-2 sm:text-left">
          <div className="rounded-2xl px-3 py-2" style={{ backgroundColor: 'var(--bg-surface-secondary)' }}>
            <div className="text-[11px] uppercase tracking-[0.18em]" style={{ color: 'var(--text-tertiary)' }}>Ajan</div>
            <div className="text-lg font-black" style={{ color: 'var(--text-primary)' }}>{formatNumber(totalAgents)}</div>
          </div>
          <div className="rounded-2xl px-3 py-2" style={{ backgroundColor: 'var(--bg-surface-secondary)' }}>
            <div className="text-[11px] uppercase tracking-[0.18em]" style={{ color: 'var(--text-tertiary)' }}>Aktif</div>
            <div className="text-lg font-black" style={{ color: 'var(--text-primary)' }}>{formatNumber(totalActiveTickets)}</div>
          </div>
          <div className="rounded-2xl px-3 py-2" style={{ backgroundColor: 'var(--bg-surface-secondary)' }}>
            <div className="text-[11px] uppercase tracking-[0.18em]" style={{ color: 'var(--text-tertiary)' }}>24 saatte çözüm</div>
            <div className="text-lg font-black" style={{ color: 'var(--text-primary)' }}>{formatNumber(totalResolvedLast24Hours)}</div>
          </div>
          <div className="rounded-2xl px-3 py-2" style={{ backgroundColor: 'var(--bg-surface-secondary)' }}>
            <div className="text-[11px] uppercase tracking-[0.18em]" style={{ color: 'var(--text-tertiary)' }}>Ort. CSAT</div>
      ...
      <div className="overflow-hidden rounded-2xl border" style={{ borderColor: 'var(--border-color-light)' }}>
        <div className="grid grid-cols-[72px_minmax(180px,1.5fr)_110px_110px_120px_100px_120px] gap-0 border-b px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.18em]" style={{ backgroundColor: 'var(--bg-surface-secondary)', borderColor: 'var(--border-color-light)', color: 'var(--text-tertiary)' }}>
          ...
          <div className="text-right">Çözülen</div>
          <div className="text-right">Ort. çözüm</div>
      ...
      {!loading && agents.length === 0 && (
        <div className="rounded-2xl border border-dashed px-4 py-10 text-center text-sm" style={{ backgroundColor: 'var(--bg-surface-secondary)', borderColor: 'var(--border-color)', color: 'var(--text-secondary)' }}>
          Görüntülenecek agent performans kaydı yok.
        </div>
      )}
    </section>
  );
